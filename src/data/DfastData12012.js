export const Value = [
    ["40.1", "20.0"],
    ["4.0", ""],
    ["56.7", ""],
    ["1.2", ""],
    ["21.1", ""],
    ["13.4", ""],
    ["-51.3", "2.6"],
]

export const Scaling = [
    ["Billions", "Actual"],
    ["Billions", ""],
    ["Billions", ""],
    ["Billions", ""],
    ["Billions", ""],
    ["Billions", ""],
    ["Billions", "Actual"],
]

export const InterimType = [
    ["Annual", "Annual"],
    ["Annual", ""],
    ["Annual", ""],
    ["Annual", ""],
    ["Annual", ""],
    ["Annual", ""],
    ["Annual", "Annual"],
]

export const PeriodDate = [
    ["03/13/2012", "03/13/2012"],
    ["03/13/2012", ""],
    ["03/13/2012", ""],
    ["03/13/2012", ""],
    ["03/13/2012", ""],
    ["03/13/2012", ""],
    ["03/13/2012", "03/13/2012"],
]

export const Unit = [
    ["USD-US Dollar", "Percentage"],
    ["USD-US Dollar", ""],
    ["USD-US Dollar", ""],
    ["USD-US Dollar", ""],
    ["USD-US Dollar", ""],
    ["USD-US Dollar", ""],
    ["USD-US Dollar", "Percentage"],
]

export const FinancialConcept = [
    ["Pre-Provision Net Revenue", "Pre-Provision Net Revenue Rate"],
    ["Other Revenue", ""],
    ["Provision for Loan and Lease Losses", ""],
    ["Credit Losses on Investment Securites (AFS/HTM)", ""],
    ["Trading and Counterparty Losses", ""],
    ["Other Gains(Losses)", ""],
    ["Net Income Before Taxes", "Net Income Before Taxes Rate"],
]

export const TOS = [
    ["Supervisory Severely Adverse", "Supervisory Severely Adverse"],
    ["Supervisory Severely Adverse", ""],
    ["Supervisory Severely Adverse", ""],
    ["Supervisory Severely Adverse", ""],
    ["Supervisory Severely Adverse", ""],
    ["Supervisory Severely Adverse", ""],
    ["Supervisory Severely Adverse", "Supervisory Severely Adverse"],
]

export const DataClassification = [
    ["Original", "Original"],
    ["Original", ""],
    ["Original", ""],
    ["Original", ""],
    ["Original", ""],
    ["Original", ""],
    ["Original", "Original"],
]

export const EntityID = [
    ["000R3K-E", "000R3K-E"],
    ["000R3K-E", ""],
    ["000R3K-E", ""],
    ["000R3K-E", ""],
    ["000R3K-E", ""],
    ["000R3K-E", ""],
    ["000R3K-E", "000R3K-E"],
]

export const ExerciseQuarter = [
    ["Not Applicable", "Not Applicable"],
    ["Not Applicable", "Not Applicable"],
    ["Not Applicable", "Not Applicable"],
    ["Not Applicable", "Not Applicable"],
    ["Not Applicable", "Not Applicable"],
    ["Not Applicable", "Not Applicable"],
    ["Not Applicable", "Not Applicable"],
]

export const ACD = [
    ["Q4 2011 - Q4 2013", "Q4 2011 - Q4 2013"],
    ["Q4 2011 - Q4 2013", ""],
    ["Q4 2011 - Q4 2013", ""],
    ["Q4 2011 - Q4 2013", ""],
    ["Q4 2011 - Q4 2013", ""],
    ["Q4 2011 - Q4 2013", ""],
    ["Q4 2011 - Q4 2013", "Q4 2011 - Q4 2013"],
]